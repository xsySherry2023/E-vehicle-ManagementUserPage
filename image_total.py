...
def image2(dataframeO, dataframeV):
    ...
    # '2022-10-24 00:00:00' to '2022-10-30' i represent day, j represent each 3-hour time period (24/3=8) 
    ...
    Vehicle_Number_ijlist = [[j, i, Vehicle_Number[i*8 + j]] for i in range(7) for j in range(8)]


    x_label = ["0-3am","3-6am","6-9am","9-12am",
...
def image3(dataframeO, dataframeV, tablename):
    ...
    for i in range(3):
        for j in range(3):
            ...
            # View the rental time of 0-10 minutes, 10-20 minutes and 20-30 minutes respectively.
            ...
            temp_list.extend(0 for _ in range(7-len(temp_list))) #append(0*(t)))  # complete list into 7 items
            
            #add up the following elements
            sum_of_rest = sum(temp_list[3:])
            temp_list = temp_list[:3] + [sum_of_rest]
            
            temp_list = [
                        {"source": "rentCityLocationName: block"+str(i+1), "target": "returnCityLocationName: block"+str(index), "value": value/sum_temp_list*100 }
    ...
    for link in links:
        if link['source'] == "rentCityLocationName: block3":
            link['lineStyle'] = opts.LineStyleOpts(color='rgba(185, 130, 238, 0.7)')
        elif link['source'] == "rentCityLocationName: block2":
            link['lineStyle'] = opts.LineStyleOpts(color='rgba(120, 120, 128, 0.8)')
...
def image5(dataframeO, dataframeV):
    ...
    data = {'x': attr,
            '时长': dict(zip(list1, list2))    #length of time
            }
